...
4. **Implement the task** completely — no stubs, no TODOs, no placeholders
5. **Validate** — run `bun run check` (build + lint + test)
...
- Up to 50 parallel Sonnet subagents
...
This should run: `bunx tsc --noEmit && bun run lint && bun run test`
