...
if [[ "$HARNESS" == "cursor" ]] && [[ -z "$MODEL" ]]; then
    echo -e "${RED}Error: --model is required for cursor harness${NC}"
    exit 1
fi
...
build_command() {
    ...
    case "$HARNESS" in
        ...
        cursor)
            # Cursor Agent CLI (print mode for non-interactive scripting)
            local cmd="cursor-agent -p --model '$MODEL'"
